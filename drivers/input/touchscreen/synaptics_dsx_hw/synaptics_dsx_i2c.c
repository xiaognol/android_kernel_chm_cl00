...
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_platform_data *pdata)
{
	...
	*/
	...
	pdata->esd_support = (bool)get_of_u32_val(np, "synaptics,esd_support", 0);
     	pdata->low_power_support = (bool)get_of_u32_val(np, "synaptics,support_low_power_model", 0);
...
static int synaptics_set_nosleep_mode(struct synaptics_rmi4_data *rmi4_data,unsigned char nosleep)
{
	...
	if (rmi4_data->board->low_power_support)
	{
		...
		retval = strcmp(rmi4_data->board->product_name, G760S_PRODUCT_NAME);  
		tp_log_debug("%s %d:product name:%s, Cherry name:%s\n",__func__, __LINE__, rmi4_data->board->product_name, G760S_PRODUCT_NAME);
